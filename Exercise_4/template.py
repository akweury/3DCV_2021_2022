...
def depth_to_3d(depth_map, kmat):
    ...
    points_img_homo = np.concatenate((xl, yl, np.ones(shape=(resized_l_img.shape[0], resized_l_img.shape[1], 1))),
                                     axis=2)
    # points_3d = points_img_homo  @ inv_K / np.expand_dims(depth_map, axis=2)
    # points_3d = (points_img_homo * np.expand_dims(depth_map, axis=2)) @ inv_K

    points_3d = np.empty(resized_l_img.shape, dtype=float)
    for i in range(resized_l_img.shape[0]):
        for j in range(resized_l_img.shape[1]):
            points_3d[i, j, :] = (depth_map[i, j] * inv_K @ points_img_homo[i, j, :].reshape(3, 1)).reshape(3)
...
def mask_outliers(similiarity_scores, sim_metric, threshold):
    ...
    '''
    scores = similiarity_scores
    if sim_metric == 'ssd':
        min_scores = np.min(similiarity_scores, axis=2)
        for i in range(similiarity_scores.shape[0]):
            for j in range(similiarity_scores.shape[1]):
                if (similiarity_scores[i, j, :] < 1.5 * min_scores[i, j]).sum() > threshold:
                    # reject this pixel
                    scores[i, j, :] = -1
    else:
        max_scores = np.max(similiarity_scores, axis=2)
        for i in range(similiarity_scores.shape[0]):
            for j in range(similiarity_scores.shape[1]):
                if (similiarity_scores[i, j, :] > 0.8 * max_scores[i, j]).sum() > threshold:
                    # reject this pixel
                    scores[i, j, :] = 1
    return scores
...
def ncc(feature_1, feature_2):
    ...
    norm = np.linalg.norm(vector_feature_1, axis=-1) * np.linalg.norm(vector_feature_2, axis=-1)

    cost = np.sum(vector_feature_1 * vector_feature_2, axis=-1) / (norm + 1)
...
def pixel_matching(img_left, img_right):
    ...
    for d in range(min_disparity, max_disparity):
        ...
        cost = np.sum(np.abs(img_left - img_right_shifted), axis=2)
        costs.append(cost)
    pixel_disparity = np.argmin(np.array(costs), axis=0)

    # 2. convert estimated disparity to depth, save it to file
    pixel_depth = disparity_to_depth(pixel_disparity, baseline)
    write_depth_to_file(pixel_depth, 'pixelDisparity.jpg')

    return pixel_depth


def window_matching(img_left, img_right, sim_metric):
    left_patches = extract_pathches(img_left, patch_width)
    # right_patches = extract_pathches(img_right, patch_width)
    costs = np.empty((img_left.shape[0], img_left.shape[1], 1))
    for d in range(min_disparity, max_disparity):
        # shift the right_patches matrix by d pixels
        M = np.array([[1, 0, d], [0, 1, 0]], dtype='float64')
        img_right_shifted = cv.warpAffine(img_right, M, (img_right.shape[1], img_right.shape[0]))
        # img_right_shifted_50px = cv.warpAffine(img_right,
        #                                        np.array([[1, 0, -50], [0, 1, 0]], dtype='float64'),
        #                                        (img_right.shape[1], img_right.shape[0]))
        # cv.imshow('right shifted image', img_right_shifted_50px)
        # cv.waitKey(0)
        ...
        if sim_metric == 'ssd':
            cost = ssd(left_patches, shifted_right_patches)
        else:
            cost = ncc(left_patches, shifted_right_patches)

        costs = np.append(costs, np.expand_dims(cost, axis=2), axis=2)
...
def stereo_matching(img_left, img_right, patch_width):
    ...
    colors = img_left.reshape((points_3d.shape[0] * points_3d.shape[1], 3))
